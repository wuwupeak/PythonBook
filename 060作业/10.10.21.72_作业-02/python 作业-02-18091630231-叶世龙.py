...
are up, to catch the waves of optimism, there is hope you may die young at eighty.'''
# 将文字中的","与“。”变为空格。
wordOne = word.replace(",","")
wordTwo = wordOne.replace(".","")
# 拆分单词进入列表中。
wordList = wordTwo.split()
# 利用将列表转集合去掉列表中多余的元素
wordSet = set(wordList)
wordListTwo = list(wordSet)

wordDirt = {}
# 在wordListTwo列表中循环。
for element in wordListTwo:
    wordnumber = 0
    # 在wordList列表中循环并与wordListTwo列表对应。
    for wordelement in wordList:
        if element == wordelement:
            wordnumber = wordnumber + 1
            frequency = wordelement
    # 将frequency作为键，wondernumber作为值加入字典
    wordDirt[frequency] = wordnumber
print(wordDirt)
