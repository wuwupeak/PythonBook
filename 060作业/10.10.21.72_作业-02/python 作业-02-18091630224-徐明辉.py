...
Sentence = """When the aerials are down, and your spirit is covered with snows of cynicism and 
the ice of pessimism, then you are grown old, even at twenty, but as long as your aerials 
are up, to catch the waves of optimism, there is hope you may die young at eighty."""
SentenceCleaned = Sentence.replace(",","").replace(".","")       # 用空格替换"," "."
wordsList = SentenceCleaned.split()         # 以空格划分每个单词

wordsUniqueSet = set(wordsList)
...
words = input('请输入单词：')
getResult = wordsCountDict.get(words,0)

if getResult == 0:
    export = False
else:
    export = words + '在文章中出现的次数是：' + str(getResult)
print(export)
