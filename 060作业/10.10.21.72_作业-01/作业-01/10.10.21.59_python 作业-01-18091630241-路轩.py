...
wordList = wordName.split()    #用split内置函数分成多个字符串组成的列表
wordNumber1 = len(wordList)    #len计算列表长度
...
for word in wordList:
    if word not in newwordList:
        newwordList.append(word)  #添加元素到newwordlist
wordunmber3 = len(newwordList)    #len计算
print("这段话总共有" + str(wordunmber3) + "个独一无二的单词")
...
for word in newwordList:
    wordNumber2 = newwordList.count(word)   #用count统计出现频数
    print(word + "在文章中出现的次数是" + str(wordNumber2))
