...
#标准字符串
tordtxt = ["'","\n","\r","\t",".",'"',"_"]
for symbols in tordtxt:
        if symbols in wordtxt:
                wordtxt = wordtxt.replace(symbols,"")
# wordtxt = wordtxt.replace(",","")
# wordtxt = wordtxt.replace(".","")
# wordtxt = wordtxt.replace('"',"")
# wordtxt = wordtxt.replace('\n',"")
# wordtxt = wordtxt.replace('"',"")
wordtxt = wordtxt.lower()
print(wordtxt)
wordlist = []
wordlist = wordtxt.split(" ") #用空格拆分字符串
...
for word in newwordlist:  #便利 newwordlist
    ...
    print(str(word) + "在文章中出现的次数是:" + str(wordsnumber))
