...
# 输出格式为"这段话总共有**个单词。"
World = '''When the aerials are down, and your spirit is covered with snows of cynicism and 
the ice of pessimism, then you are grown old, even at twenty, but as long as your aerials 
are up, to catch the waves of optimism, there is hope you may die young at eighty.'''
Worlds = World.split()
...
peoples = []
for people in World:
    if people not in World:
        peoples.append(people)
NumberWorld = len(peoples)
print("这段话总共有" + str(NumberWorld) + "个独一无二的单词。")       
...
Worlds = World.split()
for word in Worlds:
    worldNumber = Worlds.count(word)
    print(word + '在文章中出现的次数是:' + str(worldNumber))
